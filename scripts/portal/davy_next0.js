...
function enter(pi) {
	...
	if (playerS == false) {
		...
		pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, "Only the party leader may enter this portal."));
		return false;
	}else if (mobCount < 1) {
		...
		pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, "Please kill all monsters before proceeding."));
		return false;
	}
}