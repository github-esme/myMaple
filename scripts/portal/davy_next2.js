...
function enter(pi) {
	...
	var playerStatus = pi.isLeader();
		...
		if (playerStatus == false) {
			pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, "Only the party leader may proceed."));
			return false;
		}else if (mobCount !=0 || reactorCount < 8) {
			// do nothing; send message to player
			pi.getPlayer().getClient().getSession().write(Packages.tools.MaplePacketCreator.serverNotice(6, "Kill all Pirates before proceeding."));
			return false;
		}else {
