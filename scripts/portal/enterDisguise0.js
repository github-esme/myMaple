...
function enter(pi) {
	if(pi.isQuestStarted(20301) || pi.isQuestStarted(20302) || pi.isQuestStarted(20303) || pi.isQuestStarted(20304) || pi.isQuestStarted(20305)) {
		if(pi.hasItem(4032179)) {
			if (pi.getJobId() >= 1000) { pi.getPlayer().dropMessage(5, "The Erev search begins."); }
			pi.playPortalSound();
	...
	} else if (pi.getJobId() >= 1000 || pi.getJobId() == 910) {
		pi.playPortalSound();
		pi.warp(130010000, "east00");
	}
	else {
		pi.getPlayer().dropMessage(5, "Only Cygnus Knights may enter.");
	}
	return true;
}