...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 10)
            cm.sendNext("Want to be a bowman? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10 and your DEX at least 25#k. Let's see.");
        else {
    ...
    if (actionx["1stJob"]){
        ...
        if (status == 0){
            ...
            if (!cm.haveItem(4031012)){
                ...
                if (selection == 0){
                    cm.sendNext("Hunters specialize in bows. They have a variety of skills to help attack, evade danger, buff their damage, and launch arrows at groups of monsters. Hunters can attack an enemy horde with arrow barrages, but are strongest attacking single targets from afar, as their long range is balanced by modest HP.");
                    status -= 2;
                } else if (selection == 1) {
                    cm.sendNext("Crossbowmen specialize in the crossbow. They have a variety of skills to help attack, evade danger, buff their damage, and launch arrows at groups of monsters. Crossbowmen can attack an enemy horde with arrow barrages, but are strongest attacking single targets from afar, as their long range is balanced by modest HP.");
                    status -= 2;
                } else
