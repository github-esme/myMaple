...
//** Magician 3rd Job Instructor Robeira
	...
	if (actionx["Mental"]){
		if (status == 0)
			...
			if (cm.getPlayer().getRemainingSp() > 0)
				...
				var jobTitle = "";
				if(job == 211)
					jobTitle = "Fire/Poison Mage";
				else if(job == 221)
					jobTitle = "Ice/Lightning Mage";
				else if(job == 231)
					jobTitle = "Priest";

			//Need thieves text.
			cm.sendNext("You're #b" + jobTitle + "#k from here on out. May #rOdin#k be with you!");
		} else if (status == 3) {
		...
		cm.sendOk("Go, talk with #b#p1032001##k and bring me #b#t4031057##k.");
		cm.dispose();
	} else if (cm.getPlayer().gotPartyQuestItem("JBQ") && selection == 0){
		cm.sendOk("Go, talk with #b#p2030006##k and bring me #b#t4031058##k.");
		cm.dispose();
	} else {
		if (sel == undefined)
			sel = selection;
