...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 10)
            cm.sendNext("Want to be a warrior? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10 and your STR at least 35#k. Let's see.");
        else {
        ...
        if(cm.isQuestStarted(28195))
        {
            cm.gainExp(300);
            cm.forceCompleteQuest(28195);
    ...
    if (actionx["1stJob"]){
        ...
        if (status == 0){
            ...
            if (!cm.haveItem(4031012)){
                ...
                if (selection == 0){
                    cm.sendNext("Fighters specialize in 1-2 handed swords and axes and/or shields. They utilize combo strikes to create powerful blows to shake the enemy. Their third job is Crusader, and their fourth job is Hero.");
                    status -= 2;
                } else if (selection == 1) {
                    cm.sendNext("Pages are armed with swords, blunt weapons, and shields, and are naturally more defensive than the other two warrior classes. They utilize the power of fire, ice, lightning, and holy elements to scar the battlefield. Their third job is White Knight, and their fourth job is Paladin. They can equip 2-handed sword/blunt-weapon but they will be not utilizing their defensive skills without their shield being equipped. Only 1-handed sword/blunt-weapon aka mace can be equipped with shield.");
                    status -= 2;
                } else if (selection == 2) {
                    cm.sendNext("Spearmen are warriors who utilizes Spear or Polearm to attack their enemy. They use a variety of supportive buffs to support their party members, as well as their own. Some of their skills also consume HP.");
                    status -= 2;
                } else
