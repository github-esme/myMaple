 ...
 */
 ...
 var player;
 var em;
 var pinkbean = MapleExpeditionType.PINKBEAN;
 ...
 function action(mode, type, selection) {
    ...
    player = cm.getPlayer();
    expedition = cm.getExpedition(pinkbean);
    ...
    if (status == 0) {
        ...
        if (player.getClient().getChannel() != 1) { //Only channel 1
            cm.sendOk("Sorry, Pink Bean may only be challenged on #bChannel 1#k.");
            cm.dispose();
        } else if (player.getLevel() < 119) { //Don't fit requirement
        cm.sendOk("You do not meet the criteria to take on Pink Bean! You have to be at least level 120.");
        cm.dispose();
        } else if (expedition == null) { //Start an expedition
            cm.sendSimple("Would you like to assemble a team to take on the mighty #rPink Bean#k?\r\n#b#L1#Lets get this going!#l\r\n\#L2#No, I think I'll wait a bit...#l");
            status = 1;
        ...
        status = 2;
            ...
            if (expedition.contains(player)) { //If you're registered, warp you in
                em.getInstance("PinkBeanBattle_" + player.getClient().getChannel()).registerPlayer(player);
            cm.dispose();
            } else { //If you're not in by now, tough luck
            cm.sendOk("Another expedition has taken the initiative to fight Pink Bean, lets pray for those brave souls.");
            cm.dispose();
        }
    ...
    if (selection == 1) {
        cm.createExpedition(pinkbean);
        cm.sendOk("The #rPink Bean Expedition#k has been created.\r\n\r\nTalk to me again to view the current team, or start the fight!");
        cm.dispose();
        return;
    } else if (selection == 2) {
        cm.sendOk("Sure, not everyone's up to challenging the might of Pink Bean.");
        cm.dispose();
        return;
    }
} else if (status == 2) {
    if (selection == 1) {
        ...
        cm.sendOk("The expedition will begin and you will be escorted to the #bTwilight of Gods#k.");
        status = 4;
    } else if (selection == 3) {
