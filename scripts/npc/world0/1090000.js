...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 10)
            cm.sendNext("Want to be a pirate? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10 and your DEX at least 20#k. Let's see.");
        else {
...
function action(mode, type, selection) {
    ...
    if (actionx["1stJob"]){
        if (status == 0)
            ...
            cm.sendNextPrev("So be it! Now go, and go with pride.");
        ...
        if (status == 0){
            ...
            if (!cm.isQuestCompleted(2191) && !cm.isQuestCompleted(2192)){
                ...
                if (selection == 0) {
                    cm.sendNext("Brawler is one of Pirate subclass that uses knuckles to knock-down their enemies in close-range combat. They have mix traits of a Warrior (close combat) and Thief (agile attacks).");
                    status -= 2;
                } else if (selection == 1) {
                    cm.sendNext("Gunslinger is one of Pirate subclass that uses guns to shoot down their enemies in long-range combat. They have mix traits of a Bowman (far range combat) and Thief (agile attacks).");
                    status -= 2;
                }
                else
