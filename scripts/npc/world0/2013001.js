		...
		if (curMap == 1) { 
			if (playerStatus) { 
				...
				if (status == 0) {
					...
					if (cleared == null) {
						if(!cm.hasItem(4001063, 20))
						{
							cm.sendOk("You do not have 20 #v4001063# #t4001063#!");
							cm.dispose();
							return;
						}
...
function clear(stage, eim, cm) {
	...
	var packetef = Packages.tools.MaplePacketCreator.showEffect("quest/party/clear");
	var packetsnd = Packages.tools.MaplePacketCreator.playSound("Party1/Clear");
	var packetglow = Packages.tools.MaplePacketCreator.environmentChange("gate",2);
...
function failstage(eim, cm) {
	var packetef = Packages.tools.MaplePacketCreator.showEffect("quest/party/wrong_kor");
	var packetsnd = Packages.tools.MaplePacketCreator.playSound("Party1/Failed");
	var map = eim.getMapInstance(cm.getPlayer().getMapId());
	map.broadcastMessage(packetef);
	map.broadcastMessage(packetsnd);
}
        ...
        if (curMap == 6) { //SR
            if (playerStatus) { // leader
            	if (status == 0) {
            		...
            		if (preamble == null) {
            			cm.sendNext("This and the closed room of the Goddess Tower. And the room where the Goddess Minerva felt safe enough to keep it very valuable belongings. The three steps that you see at work on the side, as the locks that can open the curse, and they all have to take the right amount of weight. Let's see ... it will require five of you to be in it to match the ideal weight. You will need to solve this in seven attempts or less, or you will be banned from the sealed room.");
            			...
            			if (complete != null) {
            				...
            				cm.sendNext("Please go back to the center of Torre and continue the mission.");
            			}
            			else {
            				var totplayers = 0;
            				...
            				if (totplayers == 5 /*|| debug*/) {
            					var combo = curcombo[parseInt(eim.getProperty("stage" + nthtext + "combo"))];
                                                ...
                                                if (testcombo || debug) {
                                                    	...
                                                    	if (tries > 6) {
                                                    		cm.mapMessage(5, "You were banned from Sealed room, and the combination should be reinstated.");
                                                	...
                                                	if (debug) {
                                                		var outstring = "The objects contain:"
                                                		for (i = 0; i < objset.length; i++) {
                                                	...
                                                	cm.sendNext("It seems you have not found the platforms yet, keep trying!");
                                                	cm.dispose();
                                                }
                                            }
                                        }
