...
function start() {
    if (cm.getPlayer().gotPartyQuestItem("JBQ") && !cm.haveItem(4031058))
        if (cm.haveItem(4005004)) {
            ...
            cm.sendOk("Bring me a #b#t4005004##k to proceed with the questions.");
    cm.dispose();
}
...
function action(mode, type, selection) {
    ...
    if (status > 0)
        if (selection != ans[rand]) {
            cm.sendOk("You have failed the question.");
            cm.dispose();
            return;
        }
    ...
    if (status <= 4) {
        ...
        cm.sendOk("Alright. All your asnwers have been proven as the truth. Your wisdom has been proven.\r\nTake this necklace and go back.");
