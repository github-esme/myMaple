...
function start() {
    cm.sendOk("Pirate PQ is unavailable.");
    cm.dispose();
    return;
        ...
        if (status == 0) 
        {
            ...
            if (!cm.isLeader()) 
            {
                ...
                if (next) 
                {
                    ...
                    else if (em.getInstance("PiratePQ_" + cm.getPlayer().getClient().getChannelServer().getId().toString()) != null)
                    {
                ...
                {
                    cm.sendOk("Make sure all your members are present and qualified to participate in this quest.  I see #b" + levelValid.toString() + " #kmembers are in the right level range, and #b" + inMap.toString() + "#k are in my map. If this seems wrong, #blog out and log back in,#k or reform the party.");
                    cm.dispose();
                }
            }
        }
