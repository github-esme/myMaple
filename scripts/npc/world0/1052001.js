...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 10)
            cm.sendNext("Want to be a thief? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 10 and your DEX at least 25#k. Let's see.");
        else {
        ...
        cm.sendOk("Please, bring me the #b#t4031059##k.");
        cm.dispose();
    } else if (cm.haveItem(4031059) && cm.getPlayer().gotPartyQuestItem("JBP")){
        actionx["3thJobC"] = true;
        cm.sendOk("Nice work. You have defeated my clone and brought #b#t4031059##k back safely. You have now proven yourself worthy of the 3rd job advancement from the physical standpoint. Now you should give this necklace to #b#p2020011##k in Ossyria to take on the second part of the test. Good luck. You'll need it.");
    ...
    if (actionx["1stJob"]){
        if (status == 0)
            ...
            if (cm.canHold(2070000) && cm.canHold(1472061)){
                ...
                cm.sendOk("Make some room in your inventory and talk back to me.");
                cm.dispose();
            }
        } else if (status == 2)
        ...
        if (status == 0){
            ...
            if (!cm.haveItem(4031012)){
                ...
                if (selection == 0){
                    cm.sendNext("Assassins are users of Claws & Shurikens/Ninja stars. The are alike Bowmen in a sense that they need a supply of ammunition. Unlike Bowmen, Assassins have a variety of different kinds of ammo which don’t often from most foes but some can still be bought. This can be somewhat a hassle since they need to upgrade both Claw and Throwing Star to deal their maximum damage. Some throwing-star have higher damage and can be used at lower level, allowing them to deal higher damage than most jobs within their level range. They are also the most common job in MapleStory. Assassins are the opposite of Bandits where most of their primary skill focuses on 1 target but inflicting heavier damage which is great for boss raids.");
                    status -= 2;
                } else if (selection == 1) {
                    cm.sendNext("Possessing extremely unique skills themed around meso use, shadow abuse, and flashy dagger acrobatics, Bandits enjoy high damage and very high defense, second to only Paladins. Bandits are excellent mobbers and somewhat niche but effective bossers. However, our early jobs are painful leveling due to the lack of a decent mobbing skill until 3rd job. Our damage is high, but it is somewhat tricky to maximize the damage Bandits can deal due to expensive equips, slow rebuffs, and having clunky, delayed skills. Despite these shortcomings, Bandit is a fantastic class.");
                    status -= 2;
                } else
