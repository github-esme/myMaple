...
function action(mode, type, selection) {
    ...
    if(status == 0) {
        ...
        if(section == 0) {
            ...
            cm.sendSimple("Which item would you like to donate?"+menu);
        }
    } else if(status == 3) {
        if(section == 0) {
            if(selection == 0) {
            ...
            if(!cm.haveItem(itemID[temp])) {
                cm.sendOk("I don't think you have the item.");
                cm.dispose();
            } else {
                var quantity = cm.itemQuantity(itemID[temp]);

                cm.sendGetNumber("How many #b#t"+itemID[temp]+"#k's would you like to donate?\r\n#b< Owned : "+quantity+" >#k",0,0,quantity);
            }
        }
    } else if(status == 4) {
        if(section == 0) {
        ...
        if(count == 0 || !cm.haveItem(itemID[temp],count)) {
            cm.sendOk("Please check and see if you have enough of the item.");
        } else {
