...
function start() {
    if (cm.getJobId() == 0) {
        actionx["1stJob"] = true;
        if (cm.getLevel() >= 8)
            cm.sendNext("Want to be a magician? There are some standards to meet. because we can't just accept EVERYONE in... #bYour level should be at least 8 and your INT at least 20#k. Let's see.");
        else {
        ...
        cm.sendOk("Please, bring me the #b#t4031059##k from my clone. You can find him inside a hole in space which is deep in a forest of evil.");
        cm.dispose();
    ...
    if (actionx["1stJob"]){
        ...
        if (status == 0){
            ...
            if (!cm.haveItem(4031012)){
                ...
                if (selection == 0) {
                    cm.sendNext("Fire/Poison Magicians deals the highest amount of magic damage among all magician classes. They are able to poison enemies which deals damage overtime and set them ablaze. Enemies who are weak against Fire and Poison will receive additional damage from their attacks. However, enemies that are strong against these elements will receive less damage. Therefore, their skills acts as a a double edge sword. Fire/Poison Mage must decide their training ground wisely to reap benefit from their elemental skills.");
                    status -= 2;
                } else if (selection == 1) {
                    cm.sendNext("Ice/Lightning Arch Mages is a mix bag of support and fire-power. You can see them as the fence between Fire/Poison Arch Mage and Bishop. They deal slightly less damage than Fire/Poison and have some self-support skill acting as its defense such as ice skills to freeze enemies from retaliating. Enemies who are weak against Ice and Lightning will receive additional damage from their attacks. However, enemies that are strong against these elements will receive less damage.");
                    status -= 2;
                } else if (selection == 2) {
                    cm.sendNext("Clerics has the highest amount of supportive skills at their disposal in healing allies, dispelling negative spell effects and buffing their stats to make them beefier. This allows his/her party members to take on difficult challenges that provides better rewards compared to soloing in activities such as boss raids or to effectively speed up leveling progress. However, this job deals the least amount of damage compared to all classes, therefore, it will be a little difficult for them to train alone against powerful enemies with strong defenses. Bishop have some fondness with dark monsters, giving them an edge in dealing extra damage against them.");
                    status -= 2;
                } else
