	...
	if (status == 0) {
		...
		if (selection == 0) {
			if (cm.getParty() == null) {
				...
				cm.dispose();
				return;
			} else if (!cm.getPlayer().isIntern() && (cm.getParty().getMembers().size() < minPartySize || !cm.isLeader())) {
				cm.sendOk("You cannot participate in the quest, because you do not have at least 2 party members.");
				cm.dispose();
				return;
			} else {
