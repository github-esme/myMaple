		...
		if (mode == 1)
		 ...
		 if (cm.getPlayer().getMapId() == 980000010) {
	          ...
	          if (cm.getPlayer().getParty() != null) {
		        ...
		        switch (rnk) {
					...
					case 4:
						...
						break;
					default:
						cm.playerMessage(5, "Wow you lost but still got 300 points?");
						cm.warp(980000000, 0);
						cm.gainExp(10000);
						cm.dispose();
						break;
	        ...
	        if (status == 0) {
	            ...
	            switch (rnk) {
	                ...
	                case 4:
	                    ...
	                    break;
					default:
						cm.playerMessage(5, "You have gained over 300 points, nice!");
						cm.warp(980000000, 0);
	        ...
	        if (status == 0) {
	            ...
	            if (selection == 0) {
	                if ((cm.getLevel() > 29 && cm.getLevel() < 71) || cm.getPlayer().isIntern()) {
	                    cm.getPlayer().saveLocation(SavedLocationType.MONSTER_CARNIVAL);
	                    cm.warp(980000000, 0);
	                    cm.dispose();
	                    return;
	                } else {
	                    cm.sendOk("I'm sorry, but only the users within Level 30~70 may\r\n participate in Monster Carnival.");
	                    cm.dispose();
	                    return;
	                }
	            } else if (selection == 1) {
