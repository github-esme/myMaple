...
function action(mode, type, selection)
{
    ...
    else if (status == 1)
    {
        if (selection == 0)
        {
            cm.sendOk("The idea of transmogrification came from the game World of Warcraft. It is brought to this server exclusively for the players of myMaple. You can only transmog your non-cash equip, except weapons, shields, and medals, for 5 Maple Leaf. You can choose one stat (STR, DEX, WATK, etc.) to transfer over and the percentage of the stat that it will transfer is based on your crafting level. Each crafting level is 10% so if your crafting level is 5, it will transfer over 50% of a specific stat that you choose. Equips that have been transmogrified have a Transmog tag on them which you can equip it over non-NX equips so basically, they act like a NX equip. You can not have a NX equip over a Transmogrified equip. #eDo note that transmogrification is permanent and can not be reversed. All transmogrified equips are untradeable.#n");
            cm.dispose();
        }
        else if (selection == 1)
        {
            cm.sendYesNo("In order to transmog your equip item with stat transfer, you need #d5 Maple Leaf and 5,000,000 Mesos#k. #eTransmogrified items are permanent and cannot be reversed.#n Would you like to continue?");
        }
        else if (selection == 2)
        {
            status = 4;
            cm.sendYesNo("In order to transmog your equip item without stat transfer, you need #d1 Maple Leaf and 1,000,000 Mesos#k. #eTransmogrified items are permanent and cannot be reversed.#n Would you like to continue?");
        }
        else if (selection == 3)
        {
            status = 6;
            cm.sendYesNo("In order to copy stats on your transmogrified equip to a NX equip, you need #d1 Maple Leaf and 1,000,000 Mesos#k. #eStat copy is permanent and cannot be reversed.#n Would you like to continue?");
        }
        else if (selection == 4)
        {
            status = 9;
            cm.sendYesNo("In order to copy transmogrified stats on your NX equip to a different NX equip, you need #d1 Maple Leaf and 1,000,000 Mesos#k. #eStat copy is permanent and cannot be reversed.#n Would you like to continue?");
        }
    }
    else if (status == 2) // Transmog with stat
    {
        if (cm.haveItem(4001126, 5) && cm.getMeso() >= 5000000)
        {
        ...
        {
            cm.sendOk("You either do not have 5 Maple Leaf or 5,000,000 Mesos!");
            cm.dispose();
        }
    }
    else if (status == 3)
    {
        ...
        else
        {
            ...
            if (weapon == Packages.client.inventory.MapleWeaponType.NOT_A_WEAPON && !cm.isShield(chosenItem.getItemId())) 
            {
                ...
                {
                    percentage = cm.getPlayer().getCraftingLevel() + "0%";
                }
    ...
    {
        if (cm.haveItem(4001126, 1) && cm.getMeso() >= 1000000)
        {
            cm.sendSimple("Choose which equip you want to transmog! We won't refund your #rEquip#k if you selected wrong! \r\n\r\n" + cm.EquipList(cm.getClient()));
        }
        else
        {
            cm.sendOk("You either do not have 1 Maple Leaf or 1,000,000 Mesos!");
            cm.dispose();
            return;
        }
    }
    else if (status == 6)
    {
        ...
        else
        {
            ...
            {
                Packages.server.MapleInventoryManipulator.removeFromSlot(cm.getClient(), Packages.client.inventory.MapleInventoryType.EQUIP, selectedSlot, 1, false);
                cm.transmogItem(chosenItem.getItemId(), 0);
