    ...
    function start() {
        if (cm.getPlayer().gotPartyQuestItem("JBP") && !cm.haveItem(4031059)) {
            ...
            if(em != null)
            {
                ...
                if (cm.getJobId() >= 110 && cm.getJobId() <= 130)
                    //cm.warp(108010300);
                ...
                else if (cm.getJobId() >= 310 && cm.getJobId() <= 320)
                    //cm.warp(108010100);
                    em.newInstance(cm.getPlayer().getName()).registerPlayer(cm.getPlayer());
                //else if (cm.getPlayer().getMapId() == 100040106 && (cm.getJobId() >= 210 && cm.getJobId() <= 230))
                else if (cm.getJobId() >= 210 && cm.getJobId() <= 230)
                    //cm.warp(108010200);
                    em.newInstance(cm.getPlayer().getName()).registerPlayer(cm.getPlayer());
                ...
                else if (cm.getJobId() >= 510 && cm.getJobId() <= 520)
                    //cm.warp(108010500);
                    em.newInstance(cm.getPlayer().getName()).registerPlayer(cm.getPlayer());
            }
            else
        ...
        {
            cm.sendOk("This is not the Door of Dimension for your job. Go to your respective Door of Dimension.");
            cm.dispose();
        }
        /*20 minutes*/
    }
