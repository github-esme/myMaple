...
//        return ret;
    ...
    public int getItemID(String name) {
        ...
        for (MapleData eqpType : itemsData.getChildren()) {
            for (MapleData itemFolder : eqpType.getChildren()) {
                if (MapleDataTool.getString("name", itemFolder, "NO-NAME").equalsIgnoreCase(name)) {
                    return Integer.parseInt(itemFolder.getName());
                }
            }
        }
    ...
    public short getSlotMax(MapleClient c, int itemId) {
        ...
        if (item != null) {
            ...
            if (smEntry == null) {
                ...
                if (itemId == 2022002 || itemId == 2022181 || itemId == 4001207) {
                    ret = 100;
                } else {
        ...
        if (equip instanceof Equip) {
            ...
            if (((nEquip.getUpgradeSlots() > 0 || isCleanSlate(scrollId)) && Math.ceil(Math.random() * 100.0) <= stats.get("success")) || isGM) {
                ...
                if (!isCleanSlate(scrollId)) {
                    if (!isGM) {
                        if (scrollId != 2041058 && scrollId != 2040727) {
                            nEquip.setUpgradeSlots((byte) (nEquip.getUpgradeSlots() - 1));
                        }
                    }
    ...
    public Equip getItemAsEquipById(int itemId) {
        return (Equip) getEquipById(itemId, -1);
    }
    ...
    Item getEquipById(int equipId, int ringId) {
        ...
        if (stats != null) {
            for (Entry<String, Integer> stat : stats.entrySet()) {
                ...
                } else if (isDropRestricted(nEquip)) {
    ...
    public Equip setDonator(Equip equip, short stat) {
        equip.setStr(stat);
        equip.setDex(stat);
        equip.setInt(stat);
        equip.setLuk(stat);
        ...
        equip.setHp(stat);
        equip.setMp(stat);
        equip.setUpgradeSlots(stat);
        // equip.setFlag((byte) 8); <- makes item untradeable
        return equip;
    }

    public Equip setTransmogStats(Equip equip, short stat, int type) {
    ...
    public boolean isDropRestricted(Item source) {
        if (((source.getFlag() & ItemConstants.UNTRADEABLE) == ItemConstants.UNTRADEABLE)) {
            return true;
        }
    ...
    public boolean isTransmogrified(Equip equip) {
        return equip.getOwner().equals("Transmog");
    }
    ...
    public boolean canWearEquipment(MapleCharacter chr, Equip equip, int dst) {
        ...
        if (!EquipSlot.getFromTextSlot(islot).isAllowed(dst, isCash(id)) && !equip.getOwner().equals("Transmog")) {
            equip.wear(false);
