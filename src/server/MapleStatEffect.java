...
public class MapleStatEffect {
        ...
        if (skill) {
            switch (sourceid) {
                ...
                case Marauder.TRANSFORMATION: //transformation
                    ...
                    break;
                case Buccaneer.SUPER_TRANSFORMATION: //super transformation
                    statups.add(new Pair<>(MapleBuffStat.WDEF, Integer.valueOf(ret.wdef)));
                    statups.add(new Pair<>(MapleBuffStat.MDEF, Integer.valueOf(ret.mdef)));
                    break;
                // MULTIPLE
    ...
    private boolean applyTo(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary, Point pos) {
        ...
        if (skill && (sourceid == Bishop.BIG_BANG)) {
            return true;
        }
        ...
        if (isMagicDoor() && !FieldLimit.DOOR.check(applyto.getMap().getFieldLimit())) { // Magic Door
            ...
            if (door.getTownPortal() != null) {
                ...
                applyto.message("You may not spawn a door because all doors in the town are taken.");
            }
        } else if (isMist()) {
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    private void applyBuffEffect(MapleCharacter applyfrom, MapleCharacter applyto, boolean primary) {
        if (!isMonsterRiding()) {
            applyto.cancelEffect(this, true, -1); // <-- Causes skills to be overwritten
        }
    ...
    public boolean isMarauderTransformation() {
        return skill && sourceid == Marauder.TRANSFORMATION;
    }

    public boolean isCorsairBattleShip() {
        return skill && sourceid == Corsair.BATTLE_SHIP;
    }

    public boolean isDarkKnightHexofBeholder() {
        return skill && sourceid == DarkKnight.HEX_OF_BEHOLDER;
    }

    public boolean isHyperBody() {
        return skill && (sourceid == Spearman.HYPER_BODY || sourceid == GM.HYPER_BODY || sourceid == SuperGM.HYPER_BODY);
    }

    public boolean isBuccaneerSuperTransformation() {
        return skill && sourceid == Buccaneer.SUPER_TRANSFORMATION;
    }

    public boolean isEnergyCharge() {
        return skill && (sourceid == ThunderBreaker.ENERGY_CHARGE || sourceid == Marauder.ENERGY_CHARGE);
    }
    ...
    private boolean isBodyPressure() {
        return skill && sourceid == Aran.BODY_PRESSURE;
    }
...
//        return morphId + 100 * chr.getGender();
        ...
        switch (morphId) {
            ...
            case 1001:
            case 1003:
                return morphId + (chr.getGender() == 1 ? 100 : 0);
        }
        return morphId;
    }
    ...
    public boolean hasNoIcon() {
        return (sourceid == 3111002 || sourceid == 3211002 || + // puppet, puppet
                sourceid == 3211005 || + // golden eagle, mystic door
