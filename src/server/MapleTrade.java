...
public class MapleTrade {
    ...
    private boolean hasOneOfKind() {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();

        for (Item item : exchangeItems) {
            if (ii.isPickupRestricted(item.getItemId()) && chr.getItemQuantity(item.getItemId(), true) > 0 && item.getOwner() != "Transmog") {
                return true;
            }
        }
    ...
    public static void completeTrade(MapleCharacter c) {
        ...
        if (partner.isLocked()) {
            ...
            if (partner.hasOneOfKind() || local.hasOneOfKind()) {
                cancelTrade(c);
                c.message("You already have a one of a kind item that you are trying to receive through trade.");
                partner.getChr().message("Your trading partner already has an item that is one of a kind.");
            }
    ...
    public static void cancelTrade(MapleCharacter c) {
        ...
        if ((c.getTrade().getPartner() != null) && (c.getTrade().getPartner().getChr() != null)) {
