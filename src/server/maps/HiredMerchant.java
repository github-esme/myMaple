...
public class HiredMerchant extends AbstractMapleMapObject {
            ...
            if (c.getPlayer().getMeso() >= price) {
                if (MapleInventoryManipulator.checkSpace(c, newItem.getItemId(), newItem.getQuantity(), newItem.getOwner())) {
                    if (MapleInventoryManipulator.addFromDrop(c, newItem, true)) {
                        FilePrinter.printError("merchantpurchases.txt", "[ " + timeStamp.format(new Date()) + " ] " + c.getPlayer().getName() + " has bought " + quantity + " of " + newItem.getItemId() + " from " + getOwner() + "'s merchant for " + price + " mesos.\r\n");
                        ...
                        if (owner != null) {
                            owner.addMerchantMesos(price - getFee(price));
                        } else {
                            try {
                                try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("UPDATE characters SET MerchantMesos = MerchantMesos + " + (price - getFee(price)) + " WHERE id = ?", Statement.RETURN_GENERATED_KEYS)) {
                                    ps.setInt(1, ownerId);
                                    ps.executeUpdate();
                                }
                            } catch (Exception e) {
                        ...
                        FilePrinter.printError("merchantdupe2.txt", "[ " + timeStamp.format(new Date()) + " ] " + c.getPlayer().getName() + " tried to buy " + quantity + " of " + newItem.getItemId() + " from " + getOwner() + "'s merchant for " + price + " mesos with their inventory full.\r\n");
                        Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("Possible Merchant Dupe: " + c.getPlayer().getName() + " tried to buy " + quantity + " of " + newItem.getItemId() + " from " + getOwner() + "'s merchant for " + price + " mesos with their inventory full."));
                        c.getPlayer().dropMessage(1, "Your inventory is full. Please clean a slot before buying this item.");
                    }
                } else {
                    FilePrinter.printError("merchantdupe.txt", "[ " + timeStamp.format(new Date()) + " ] " + c.getPlayer().getName() + " tried to buy " + quantity + " of " + newItem.getItemId() + " from " + getOwner() + "'s merchant for " + price + " mesos with their inventory full.\r\n");
                    Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip("Possible Merchant Dupe: " + c.getPlayer().getName() + " tried to buy " + quantity + " of " + newItem.getItemId() + " from " + getOwner() + "'s merchant for " + price + " mesos with their inventory full."));
                    c.getPlayer().dropMessage(1, "Your inventory is full. Please clean a slot before buying this item.");
                }
            } else {
    ...
    private int getFee(int meso) {
        ...
        if (meso >= 100000000) {
            ...
            fee = (int) Math.round(.025 * meso);
        } else if (meso >= 10000000) {
            fee = (int) Math.round(.02 * meso);
        } else if (meso >= 5000000) {
            fee = (int) Math.round(.015 * meso);
        } else if (meso >= 1000000) {
            fee = (int) Math.round(.009 * meso);
        } else if (meso >= 100000) {
            fee = (int) Math.round(.004 * meso);
        }
        return fee;
    }
