...
public class AbstractPlayerInteraction {
    ...
    public void warpParty(int id) {
        for (MapleCharacter mc : getPartyMembers()) {
            if (id == 925020100) {
                ...
                mc.setDojoPartyCount(getPartyMembers().size());
            }
            mc.changeMap(getWarpMap(id));
        }
    }

    public void warpParty(int id, int portal) {
        for (MapleCharacter mc : getPartyMembers()) {
            if (id == 925020100) {
                mc.setDojoParty(true);
                mc.setDojoPartyCount(getPartyMembers().size());
            }
            mc.changeMap(getWarpMap(id), getWarpMap(id).getPortal(portal));
        }
    }
    ...
    public boolean hasItemOrEquipped(int itemid) {
        return getPlayer().getItemQuantity(itemid, true) >= 1;
    }
    ...
    public void addGuildBuffs(String buff) {
        // Guild ID-Buff Type-% Increase-Timestamp Ending

        // Type 1 = EXP Buff
        // Type 2 = Meso Buff
        // Type 3 = Drop Buff
        getClient().getWorldServer().addGuildBuffs(buff);
    }
    ...
    public boolean forceCompleteQuest(int id) {
        return MapleQuest.getInstance(id).forceComplete(getPlayer(), 0);
    }
        ...
        if (quantity >= 0) {
            ...
            if (ii.getInventoryType(id).equals(MapleInventoryType.EQUIP) && !ItemConstants.isRechargable(item.getItemId())) {
                if (randomStats) {
                    ...
                    equip.setFlag((byte) ItemConstants.UNTRADEABLE);

                    item = ii.randomizeStats(equip);
                    MapleInventoryManipulator.addFromDrop(c, ii.randomizeStats(equip), false);
                } else {
                    Equip equip = (Equip) item;
                    equip.setFlag((byte) ItemConstants.UNTRADEABLE);
    ...
    // For Inventory Sorting
    public Equip gainEquip(Equip gear, boolean showMessage) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();

        if (!MapleInventoryManipulator.checkSpace(c, gear.getItemId(), 1, "")) {
            c.getPlayer().dropMessage(1, "Your inventory is full. Please remove an item from your " + ii.getInventoryType(gear.getItemId()).name() + " inventory.");
            return null;
        }

        MapleInventoryManipulator.addFromDrop(c, gear, false);

        if (showMessage) {
            c.announce(MaplePacketCreator.getShowItemGain(gear.getItemId(), (short) 1, true));
        }
    ...
    public void spawnMonster(int id, int quantity, int x, int y) {
        for (int i = 0; i < quantity; i++) {
            getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(id), new Point(x, y));
        }
    }
