...
public class ReactorActionManager extends AbstractPlayerInteraction {
    ...
    public void customSpawnMonster(int id, int hp, int exp) {
        ...
        reactor.getMap().spawnMonsterOnGroudBelow(monster, getPosition());
    }
    ...
    public void spawnMonsterHP(int id, int hp, int qty) {
        for (int i = 0; i < qty; i++) {
            MapleMonster mob = MapleLifeFactory.getMonster(id);
            mob.setHp(hp);

            reactor.getMap().spawnMonsterOnGroudBelow(mob, getPosition());
        }
    }

    public void spawnMonsterHP(int id, int hp, int qty, int x, int y) {
        for (int i = 0; i < qty; i++) {
            MapleMonster mob = MapleLifeFactory.getMonster(id);
            mob.setHp(hp);

            reactor.getMap().spawnMonsterOnGroudBelow(mob, new Point(x, y));
        }
    }

    public void spawnMonsterHPEXP(int id, int hp, int exp, int qty) {
        for (int i = 0; i < qty; i++) {
            reactor.getMap().spawnMonsterOnGroudBelow(MapleLifeFactory.getMonster(id), getPosition());
        }
    ...
    public void spawnMonsterHPEXP(int id, int hp, int exp, int qty, int x, int y) {
        for (int i = 0; i < qty; i++) {
            MapleMonster mob = MapleLifeFactory.getMonster(id);
            mob.setHp(hp);
            mob.setExp(exp);
    ...
    public void customSpawnFakeMonster(int id, int hp, int exp) {
        MapleMonster monster = MapleLifeFactory.getMonster(id);
