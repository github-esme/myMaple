...
public class AutobanManager {
    ...
    public void addPoint(AutobanFactory fac, String reason) {
        ...
        FilePrinter.printError("autobanwarning.txt", "[ " + timeStamp.format(new Date()) + " ] " + MapleCharacter.makeMapleReadable(chr.getName()) + " caused " + fac.name() + " " + reason + "\r\n");
    }
    ...
    public void setTimestamp(int type, int time, int times) {
        if (this.timestamp[type] == time) {
            ...
            if (this.timestampcounter[type] >= times) {
                //chr.getClient().disconnect(false, false); //<-- Causes people to DC while on chairs and while spamming teleport and heal
