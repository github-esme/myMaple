...
public class MapleClient {
    ...
    public int login(String login, String pwd) {
        ...
        ResultSet rs = null;
        try {
            ps = con.prepareStatement("SELECT id, password, salt, gender, banned, gm, pin, pic, characterslots, tos, passchange FROM accounts WHERE name = ?");
            ...
            if (rs.next()) {
                ...
                if (getLoginState() > LOGIN_NOTLOGGEDIN) { // already loggedin
                    loggedIn = false;
                    loginok = 7;
                } else if (pwd.equals(passhash) || checkHash(passhash, "SHA-1", pwd) || checkHash(passhash, "SHA-512", pwd + salt + "629XG6wx639b766b2IMV1Pn5g")) {
                    if (tos == 0) {
                        loginok = 23;
                    } else {
                        loginok = 0;
    ...
    public final void disconnect(boolean shutdown, boolean cashshop) {//once per MapleClient instance
        ...
        if (player != null && player.isLoggedin() && player.getClient() != null) {
            ...
            if (player.isTestingDPS()) {
                player.getMap().killMonsterDPS(player.getId());
                player.toggleTestingDPS();
            }

            if (player.ownsMap()) {
                player.getOwnedMap().resetMapOwner();

                player.getMap().dropMessage(6, "[Map Ownership] " + player.getName() + " has lost ownership of this map. You can type @mapowner to take ownership of this map.");

                if (player.isAFKChecked()) {
                    player.resetAFKCheck();
                }
            }
            ...
            try {
                if (!player.getDoors().isEmpty()) {
                    MapleDoor door = player.getDoors().iterator().next();
                    ...
                    if (door != null) {
                        ...
                        for (MapleDoor destroyDoor : player.getDoors()) {
                            if (destroyDoor != null) {
                        ...
                        if (player.getParty() != null) {
                            for (MaplePartyCharacter partyMembers : player.getParty().getMembers()) {
    ...
    public String getCharNameByID(int cid) {
        String name = "";
        try {
    ...
    public int getDonatorPoints() {
        int points = 0;
        try {
            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `donatorpoints` FROM accounts WHERE id = ?");
            ps.setInt(1, accId);
            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                points = rs.getInt("donatorpoints");
            }
            ps.close();
            rs.close();
        ...
        donatorPoints = points;
        return donatorPoints;
    }

    public Boolean addStreamerPointsOffline(String ign, int points) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        int currentPoints = 0;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                try (PreparedStatement psb = DatabaseConnection.getConnection().prepareStatement("SELECT streamerpoints FROM accounts WHERE id = ?")) {
                    psb.setInt(1, id);

                    ResultSet rs2 = psb.executeQuery();

                    if (rs2.next()) {
                        currentPoints = rs2.getInt("streamerpoints");
                    }
                ...
                try (PreparedStatement psb2 = DatabaseConnection.getConnection().prepareStatement("UPDATE accounts SET streamerpoints = ? WHERE id = ?")) {
                    psb2.setInt(1, currentPoints + points);
                    psb2.setInt(2, id);
                    psb2.executeUpdate();
...
//        int points = 0;
//        try {
//            PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `streamerpoints` FROM accounts WHERE id = ?");
//            ps.setInt(1, accId);
//            ResultSet rs = ps.executeQuery();
//
//            if (rs.next()) {
//                points = rs.getInt("streamerpoints");
//            }
//            ps.close();
//            rs.close();
//
...
//    }
    ...
    public Boolean addVotePointsOffline(String ign, int points) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        int currentPoints = 0;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                try (PreparedStatement psb = DatabaseConnection.getConnection().prepareStatement("SELECT votes FROM accounts WHERE id = ?")) {
                    psb.setInt(1, id);

                    ResultSet rs2 = psb.executeQuery();

                    if (rs2.next()) {
                        currentPoints = rs2.getInt("votes");
                    }
                ...
                try (PreparedStatement psb2 = DatabaseConnection.getConnection().prepareStatement("UPDATE accounts SET votes = ? WHERE id = ?")) {
                    psb2.setInt(1, currentPoints + points);
                    psb2.setInt(2, id);
                    psb2.executeUpdate();
    ...
    public void addDonatorPoints(int points) {
        donatorPoints += points;

        try {
            Connection con = DatabaseConnection.getConnection();
            try (PreparedStatement ps = con.prepareStatement("UPDATE accounts SET donatorpoints = ? WHERE id = ?")) {
                ps.setInt(1, donatorPoints);
                ps.setInt(2, accId);
                ps.executeUpdate();
            }
        } catch (SQLException e) {
    ...
    public Boolean addDonatorPointsOffline(String ign, int points) {
        PreparedStatement ps = null;
        ResultSet rs = null;
        int currentPoints = 0;
        ...
        try {
            ...
            if (rs.next()) {
                ...
                try (PreparedStatement psb = DatabaseConnection.getConnection().prepareStatement("SELECT donatorpoints FROM accounts WHERE id = ?")) {
                    psb.setInt(1, id);

                    ResultSet rs2 = psb.executeQuery();

                    if (rs2.next()) {
                        currentPoints = rs2.getInt("donatorpoints");
                    }
                ...
                try (PreparedStatement psb2 = DatabaseConnection.getConnection().prepareStatement("UPDATE accounts SET donatorpoints = ? WHERE id = ?")) {
                    psb2.setInt(1, currentPoints + points);
                    psb2.setInt(2, id);
                    psb2.executeUpdate();
    ...
    public void changeChannel(int channel) {
        ...
        if (player.isTestingDPS()) {
            player.getMap().killMonsterDPS(player.getId());
            player.toggleTestingDPS();
            player.message("DPS testing has been cancelled.");
        }
