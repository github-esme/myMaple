...
public class PlayerStorage {
    ...
    public final void fish() {
        wlock.lock();
        try {
            final Iterator<MapleCharacter> i = storage.values().iterator();
            while (i.hasNext()) {
                MapleCharacter chr = i.next();

                if (chr.isFishing()) {
                    chr.gainFishingPoint(1);

                } else if (chr.isStreaming()) {
                    chr.addStreamerPoints(1);
                    chr.gainStreamingTime(1);

                    if (chr.getStreamingTime() >= 10) {
                        chr.message("This is an automatic message that you are currently streaming. Please turn off @togglestream if you are not streaming.");
                        chr.gainStreamingTime(-chr.getStreamingTime());
                    }
                }
            }
        } finally {
