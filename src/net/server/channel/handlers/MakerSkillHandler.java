...
public final class MakerSkillHandler extends AbstractMaplePacketHandler {
            ...
            case 1:
                ...
                if (ii.getInventoryType(toCreate) == MapleInventoryType.EQUIP) {
                    ...
                    if (item != null) {
                        ...
                        c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getScrollEffect(c.getPlayer().getId(), ScrollResult.FAIL, false));
                        //c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.showSpecialEffect(16));
                        c.getPlayer().dropMessage(1, "You have failed in making the item!");
                    }
                    ...
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getScrollEffect(c.getPlayer().getId(), reward.getLeft() >= toCreate ? ScrollResult.SUCCESS : ScrollResult.FAIL, false));
                    //c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.showSpecialEffect(16));
                    c.getPlayer().dropMessage(1, reward.getLeft() >= toCreate ? "Congratulations! You've succeeded in the making of the item!" : "You have failed in making the item!");
                }
                break;
            case 3:
    ...
    private boolean canCreate(MapleClient c, MakerItemCreateEntry recipe) {
        return hasItems(c, recipe) && c.getPlayer().getMeso() >= recipe.getCost() && c.getPlayer().getLevel() >= recipe.getReqLevel() && c.getPlayer().getSkillLevel(SkillFactory.getSkill(1007 + (GameConstants.getBeginnerJob((short) c.getPlayer().getJob().getId()) * 10000))) >= recipe.getReqSkillLevel();
