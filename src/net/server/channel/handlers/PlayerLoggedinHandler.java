...
public final class PlayerLoggedinHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (player == null) {
            try {
                player = MapleCharacter.loadCharFromDB(cid, c, true); // <-- causes the lag

                if (player != null && player.getWelcome() == 0) {
                    Server.getInstance().broadcastMessage(MaplePacketCreator.serverNotice(6, "[Welcome] Let's give a warm welcome to " + player.getName() + " for joining myMaple!"));

                    Connection con = DatabaseConnection.getConnection();
                    PreparedStatement ps;
                    ps = con.prepareStatement("UPDATE characters SET welcome = ? WHERE id = ?");
