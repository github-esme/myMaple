...
//                }
            ...
            if (attack.skill == ChiefBandit.MESO_EXPLOSION) {
                ...
                for (Integer oned : attack.allDamage.keySet()) {
                    ...
                    if (mapobject != null && mapobject.getType() == MapleMapObjectType.ITEM) {
                        ...
                        if (mapitem.getOwnerWithoutParty() != player.getId()) {
                            return;
                        }
            ...
            for (Integer oned : attack.allDamage.keySet()) {
                ...
                if (monster != null) {
                    ...
                    if (distance > distanceToDetect && attack.skill != Buccaneer.DRAGON_STRIKE) {
                    ...
                    if (monster.belongsToSomeone()) {
                        if (monster.getBelongsTo() != player.getId()) {
                            return;
                        } else if (attack.skill == ChiefBandit.MESO_EXPLOSION || attack.skill == Paladin.HEAVENS_HAMMER || attack.skill == Aran.COMBO_TEMPEST) {
                            player.message("That skill attack will not affect your DPS.");
                            return;
                        }
                    }
                    ...
                    if (player.getBuffedValue(MapleBuffStat.PICKPOCKET) != null && (attack.skill == 0 || attack.skill == Rogue.DOUBLE_STAB || attack.skill == Bandit.SAVAGE_BLOW || attack.skill == ChiefBandit.ASSAULTER || attack.skill == ChiefBandit.BAND_OF_THIEVES || attack.skill == Shadower.ASSASSINATE || attack.skill == Shadower.TAUNT || attack.skill == Shadower.BOOMERANG_STEP)) {
                        ...
                        for (Integer eachd : onedList) {
                            ...
                            if (pickpocket.getEffect(player.getSkillLevel(pickpocket)).makeChanceResult()) {
                                ...
                                TimerManager.getInstance().schedule(new Runnable() {
                                    @Override
                                    public void run() {
                                        player.getMap().spawnMesoDrop(Math.min((int) Math.max((eachdf / (double) 20000) * maxmeso, 1), maxmeso), new Point((int) (monster.getPosition().getX() + Randomizer.nextInt(100) - 50), (int) (monster.getPosition().getY())), monster, player, true, (byte) 2);
                                    }
                                }, delay);
                                delay += 100;
                            }
                        ...
                        player.addHP(Math.min(monster.getMaxHp(), Math.min((int) (totDamage * (double) SkillFactory.getSkill(attack.skill).getEffect(player.getSkillLevel(SkillFactory.getSkill(attack.skill))).getX() / 100.0), player.getMaxHp() / 2)));
                    } else if (attack.skill == Bandit.STEAL) {
                        Skill steal = SkillFactory.getSkill(Bandit.STEAL);
                        if (monster.getStolen().size() < 1) { // One steal per mob <3
                            if (Math.random() < 0.1 && steal.getEffect(player.getSkillLevel(steal)).makeChanceResult()) { //Else it drops too many cool stuff :(
                    ...
                    if (attack.skill != 0) {
                        if (attackEffect.getFixDamage() != -1) {
                            if (totDamageToOneMonster != attackEffect.getFixDamage() && totDamageToOneMonster != 0 && attack.skill != Beginner.THREE_SNAILS) {
                    ...
                    if (attack.skill == Bishop.GENESIS || attack.skill == ILArchMage.BLIZZARD
                            || attack.skill == Corsair.AERIAL_STRIKE || attack.skill == DragonKnight.DRAGON_ROAR || attack.skill == Ranger.ARROW_RAIN
                            || attack.skill == Sniper.MORTAL_BLOW || attack.skill == BlazeWizard.METEOR_SHOWER
                            || attack.skill == DawnWarrior.SOUL_DRIVER || attack.skill == WindArcher.ARROW_RAIN || attack.skill == Aran.FINAL_BLOW
                            || attack.skill == Crusader.SHOUT) {
                        player.setUsedAOE(true);
                    }
...
//                }
                if (ret.skill == Marksman.SNIPE) {
                    damage = 995000 + Randomizer.nextInt(5000);
                    hitDmgMax = 1000000;
                }
...
//                if (!chr.isIntern() && !chr.hasMarauderTransformation() && !chr.hasEnergyCharge()
//                        && ret.skill != ThunderBreaker.LIGHTNING_CHARGE && ret.skill != Bowmaster.HURRICANE
//                        && ret.skill != NightWalker.VAMPIRE && ret.skill != DragonKnight.SPEAR_CRUSHER
//                        && ret.skill != DragonKnight.RUSH
//                        && ret.skill != ThunderBreaker.SOMMERSAULT_KICK
//                        && ret.skill != Pirate.SOMMERSAULT_KICK
//                        && ret.skill != ThunderBreaker.ENERGY_BLAST
//                        && ret.skill != ThunderBreaker.STRAIGHT
//                        && ret.skill != ThunderBreaker.ENERGY_CHARGE
//                        && ret.skill != Marauder.ENERGY_CHARGE
//                        && ret.skill != Paladin.BLAST
//                        && ret.skill != Priest.SHINING_RAY) {
                if (!chr.isIntern()) {
                    // Warn if the damage is over 1.5x what we calculated above.
                    if (chr.getJob().isA(MapleJob.ARAN1)) {
                        if (damage > maxWithCrit * 1.5) {
                            AutobanFactory.DAMAGE_HACK.alert(chr, "-- ARAN -- DMG: " + damage + " MaxDMG: " + maxWithCrit + " SID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null") + " Map: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")");
                        }
                    } else if (damage > maxWithCrit * 1.5) {
                    ...
                    if (damage == 1333333337) {
                        chr.autoban("DMG: " + damage + " MaxDMG: " + maxWithCrit + " SID: " + ret.skill + " MobID: " + (monster != null ? monster.getId() : "null") + " Map: " + chr.getMap().getMapName() + " (" + chr.getMapId() + ")");
                    }

                    // Add a ab point if its over 5x what we calculated.
                    // disabled by Torban because of false positives
...
//                    System.out.println("NEW DAMAGE 2: " + damage);
//                }
