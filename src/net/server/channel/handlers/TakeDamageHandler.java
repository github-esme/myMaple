...
public final class TakeDamageHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (damagefrom != -3 && damagefrom != -4) {
            ...
            if (map.getMapObject(oid) instanceof MapleMonster) {
                attacker = (MapleMonster) map.getMapObject(oid);
            }

            List<loseItem> loseItems = null;
            if (attacker != null) {
                if (attacker.isBuffed(MonsterStatus.NEUTRALISE) || player.getMap().isCPQMap()) {
                    return;
                }
                if (damage > 0) {

                    // Chaos Zakum
                    if (player.getMapId() == 280030000 && player.getEventInstance() != null && player.getEventInstance().getProperty("chaos") != null && player.getEventInstance().getProperty("chaos").equals("true")) {
                        damage = damage * 2;
                    }

                    MapleMonster mm = map.getMonsterById(monsteridfrom);

                    if (mm != null && mm.getStats() != null && mm.getStats().loseItem() != null) {
        ...
        if (damage > 0 && !player.isHidden()) {
            ...
            if (player.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null && mpattack == 0) {
                ...
                if (attacker != null) {
                    if (attacker.getTeam() == c.getPlayer().getTeam() && c.getPlayer().getMap().isCPQMap()) {
                        return;
                    }
                }

                // Aran's High Defense
                if (player.getJob().getId() == 2112) {
                    if (player.getSkillLevel(Aran.HIGH_DEFENSE) > 0) {
                        if (player.getAttackedBy() == attacker.getId()) {

                            double percent = player.getSkillLevel(Aran.HIGH_DEFENSE) * 0.5 / 100;

                            damage = damage - (int) (damage * percent);
                        } else {
                            player.setAttackedBy(attacker.getId());
                        }
                    }
                }
        ...
        if (map.getId() >= 925020000 && map.getId() < 925030000) {
            player.setDojoEnergy(player.isIntern() ? 300 : player.getDojoEnergy() < 300 ? player.getDojoEnergy() + 1 : 0); //Fking gm's
