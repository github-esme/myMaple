...
public final class EnterMTSHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        if (!c.getPlayer().isAlive()) {
            ...
            c.getPlayer().dropMessage(1, "You can not use the Trade button in an instance!");
        } else if (c.getPlayer().getLevel() < 10) {
            c.getPlayer().dropMessage(1, "You have to be at least level 10 to be teleported to the FM!");
        } else if (c.getPlayer().getSavedLocationWithoutClearing("EVENT") != -1) {
            c.getPlayer().dropMessage(1, "You can not go to FM when you're in an event! Use @leaveevent to leave the event.");
        } else if (c.getPlayer().getMapId() == 910000000) {
            c.getPlayer().dropMessage(1, "You are already in the FM!");
        } else {
            c.getPlayer().saveLocation("FREE_MARKET");
            c.getPlayer().changeMap(910000000, "out00");
        }
...
//                } else {
//                    Equip equip = new Equip(rs.getInt("itemid"), (short) rs.getInt("position"), -1);
