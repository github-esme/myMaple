...
public final class HealOvertimeHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (healHP != 0) {
            if ((abm.getLastSpam(0) + 1500) > timestamp && !MapleLogger.ignored.contains(chr.getName())) {
                //AutobanFactory.FAST_HP_HEALING.addPoint(abm, "Fast HP healing");
                Server.getInstance().broadcastGMMessage(MaplePacketCreator.sendYellowTip(chr.getName() + " caused Fast HP healing. May be a false positive."));
            }

            int abHeal = 140;
            if (chr.getMapId() == 105040401 || chr.getMapId() == 105040402 || chr.getMapId() == 809000101 || chr.getMapId() == 809000201) {
                abHeal += 10000; // Sleepywood sauna and showa spa...
            }

            if (healHP > abHeal && chr.getChair() == 0) {
        ...
        if (healMP != 0 && healMP < 1000 && chr.getChair() == 0) {
            if ((abm.getLastSpam(1) + 1500) > timestamp && !MapleLogger.ignored.contains(chr.getName())) {
