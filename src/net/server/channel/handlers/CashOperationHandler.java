...
public final class CashOperationHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (action == 0x03 || action == 0x1E) {
            ...
            if (slea.toString().replaceAll("\\s+", "").equals("30 00 02 00 45 45 09") || slea.toString().replaceAll("\\s+", "").contains("30 00 02 00 45 45 09")) {
                return;
            }
...
//            if (cItem.getItemId() >= 5211000 && cItem.getItemId() <= 5211048 || cItem.getItemId() == 5211048 || cItem.getItemId() == 1812007 || cItem.getItemId() == 5140000 || cItem.getItemId() == 5401000) {
//                c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "You may not purchase this item."));
//                c.announce(MaplePacketCreator.showCash(chr));
//                return;
//            }
            if (blocked(cItem.getItemId())) {
                c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "You may not purchase this item."));
                c.announce(MaplePacketCreator.showCash(chr));
                return;
            }
            ...
            if (blocked(cItem.getItemId()) || cItem.getItemId() == 5360042) {
                c.getPlayer().getClient().getSession().write(MaplePacketCreator.serverNotice(1, "You may not gift this item."));
                c.announce(MaplePacketCreator.showCash(chr));
                return;
            }
...
//            c.announce(MaplePacketCreator.putIntoCashInventory(item, c.getAccID()));
            if (item.getItemId() == 1112918) {
                chr.dropMessage(1, "You can not put this ring inside the cash inventory!");
