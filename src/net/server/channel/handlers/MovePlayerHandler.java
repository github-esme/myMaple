...
public final class MovePlayerHandler extends AbstractMovementPacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        if (res != null && c.getPlayer().getMap() != null) {
            if (c.getPlayer().isAFKChecked()) {
                c.getPlayer().resetAFKCheck();
                c.getPlayer().message("Since you are not AFK, you are not under AFK check anymore and is still the owner of this map.");
            }

            updatePosition(res, c.getPlayer(), 0);

            //c.getPlayer().getMap().movePlayer(c.getPlayer(), c.getPlayer().getPosition());
            c.getPlayer().movePlayer(c.getPlayer().getPosition());
