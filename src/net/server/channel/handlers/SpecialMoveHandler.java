...
public final class SpecialMoveHandler extends AbstractMaplePacketHandler {
    ...
    public final void handlePacket(SeekableLittleEndianAccessor slea, MapleClient c) {
        ...
        chr.getAutobanManager().setTimestamp(4, slea.readInt(), 3); //<-- Causes people to DC while on chairs and while spamming teleport and heal
        int skillid = slea.readInt();
        if ((!GameConstants.isPQSkillMap(c.getPlayer().getMapId()) && GameConstants.isPqSkill(skillid)) || (!c.getPlayer().isIntern() && GameConstants.isGMSkills(skillid)) || (!GameConstants.isInJobTree(skillid, c.getPlayer().getJob().getId()) && !c.getPlayer().isIntern())) {
        ...
        if (skillid == Hero.MONSTER_MAGNET || skillid == Paladin.MONSTER_MAGNET || skillid == DarkKnight.MONSTER_MAGNET) { // Monster Magnet
            ...
            if (chr.getHp() - lose <= 0) {
                chr.message("You may not use this skill right now because you will suicide due to low HP.");
            } else {
        ...
        if (chr.isAlive()) {
            if (skill.getId() == Priest.MYSTIC_DOOR) {
                chr.message("Mystic Door has been disabled.");
                c.announce(MaplePacketCreator.enableActions());
