        ...
        if (mode == Action.CREATE.getCode()) {
            ...
            if (chr.getTrade() != null) {
                if ((quantity <= item.getQuantity() && quantity >= 0) || ItemConstants.isRechargable(item.getItemId())) {
                    if (ii.isDropRestricted(item)) { // ensure that undroppable items do not make it to the trade window
            ...
            // Checks for the entire inventory for the quantity instead of the quantity of the slot which = dupes when packeting
...
//            }
            ...
            if (chr.getInventory(type).getItem(slot) == null || chr.getInventory(type).getItem(slot).getQuantity() < bundles || chr.getInventory(type).getItem(slot).getFlag() == ItemConstants.UNTRADEABLE) {
                return;
            }
            ...
            if (merchant != null && merchant.isOwner(c.getPlayer())) {
                ...
                if (merchant.getItems().size() <= 0 || merchant.getItems().size() <= slot || slot < 0) {
                    return;
                }
