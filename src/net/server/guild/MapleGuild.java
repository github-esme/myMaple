...
public class MapleGuild {
    ...
    private int id, gp, level, logo, logoColor, leader, capacity, logoBG, logoBGColor, signature, allianceId;
    private int world;
    ...
    public void writeToDB(boolean bDisband) {
        try {
            ...
            if (!bDisband) {
                ...
                builder.append("UPDATE guilds SET GP = ?, level = ?, logo = ?, logoColor = ?, logoBG = ?, logoBGColor = ?, ");
                for (int i = 0; i < 5; i++) {
    ...
    public String getRate(double base) {
        double rate = level * base;
    ...
    public void gainGuildLevel(int abc) {
        level += abc;
    ...
    public static void displayGuildRanks(MapleClient c, int npcid) {
        try {
            ResultSet rs;
            try (PreparedStatement ps = DatabaseConnection.getConnection().prepareStatement("SELECT `name`, `level`, `logoBG`, `logoBGColor`, `logo`, `logoColor` FROM guilds WHERE NOT `name` = 'Staff' ORDER BY `level` DESC LIMIT 50")) {
                rs = ps.executeQuery();
