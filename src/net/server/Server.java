...
public class Server implements Runnable {
    ...
    public void run() {
        ...
        System.out.println("myMaple v" + ServerConstants.VERSION + " starting up.\r\n");
        ...
        try {
            for (int i = 0; i < Integer.parseInt(p.getProperty("worlds")); i++) {
                ...
                world.setExpRate(4); // <-- These things are needed for some reason.
                world.setMesoRate(2);
                world.setDropRate(1);
                world.setBossDropRate(ServerConstants.BOSS_DROP_RATE);
                world.setQuestRate(ServerConstants.QUEST_RATE);

                tMan.register(server.events.Fishing.fish(), 120000);
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        System.out.println("myMaple is now online.");
        online = true;
    }
    ...
    public boolean changeAllianceCapacity(int allianceid) {
        MapleAlliance mga = alliances.get(allianceid);
        if (mga != null) {
    ...
    public boolean increaseAllianceCapacity(int aId) {
        MapleAlliance alliance = alliances.get(aId);
        if (alliance != null) {
    ...
    public final Runnable shutdown2(final boolean restart) {//only once :D
        return new Runnable() {
            @Override
            public void run() {
